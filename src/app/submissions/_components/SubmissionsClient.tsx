...
// All imports from the original file remain the same
...
const ChevronLeftIcon = () => <svg xmlns="http://www.w.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" /></svg>;
const ChevronRightIcon = () => <svg xmlns="http://www.w.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" /></svg>;
type Entry = { local_date: string; topics: string; alignment_rating: number; contentment_rating: number; id?: string; };


// This is our new client component that receives initial data as props
export default function SubmissionsClient({ initialWeekStart, initialEntries }: { initialWeekStart: string, initialEntries: Entry[] }) {
    // Initialize state with the props from the server
    const [weekStartDate, setWeekStartDate] = useState(() => new Date(initialWeekStart));
    const [data, setData] = useState<Entry[]>(initialEntries);
    const [loading, setLoading] = useState(false); // No initial loading, as data is provided
    ...
    // useSearchParams is now safe inside a Suspense boundary
    ...
    // This effect now ONLY runs for subsequent week navigations
    useEffect(() => {
        ...
        const fetchSubmissions = async () => {
            ...
            try {
                let entries: Entry[] = [];
                if (isGuest) {
                    // This filtering logic is now client-side for navigation
    ...
    // The scroll-to-highlight effect remains the same
    useEffect(() => {
        if (!highlightedDate || !entryRefs.current.has(highlightedDate)) return;

        const node = entryRefs.current.get(highlightedDate);
        if (node) {
            setTimeout(() => node.scrollIntoView({ behavior: "smooth", block: "center" }), 100);
        }
    }, [highlightedDate, data]); // Depends on data to ensure element exists

    // --- (All memoized values, week handlers, and delete handlers remain exactly the same) ---
    ...
    // --- (The entire JSX return block from the original component is copied here) ---
    return (
        <>
            ...
            {loading ? (
                // You can replace this with a more granular skeleton if you prefer
